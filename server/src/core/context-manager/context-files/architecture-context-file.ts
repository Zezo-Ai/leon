...
export class ArchitectureContextFile extends ContextFile {
  ...
  public generate(): string {
    return [
      ...
      '## Memory System',
      '- `server/src/core/memory-manager/memory-manager.ts`: memory orchestration (write, recall, summarization, pruning, context freshness tracking).',
      '- `server/src/core/memory-manager/qmd-backend.ts`: retrieval backend using the QMD CLI/index for memory and context recall.',
      '- `server/src/core/memory-manager/memory-repository.ts`: SQLite persistence for memory records/facts and lifecycle operations (not retrieval ranking).',
      '- `server/src/core/memory-manager/sql/schema.sql`: relational schema for memory items/facts and lifecycle metadata.',
      ...
      '- Query-aware recall: planning and execution build token-budgeted memory packs from QMD results.',
      '- Context RAG integration: context files are indexed by QMD; execution recall is filtered to toolkit-declared context files.',
